...
int main(int argc, char* argv[]) {
    ...
    if (rank == 0) {
        ...
        arma::Mat<real> y =
            arma::zeros<arma::Mat<real>>(NUM_CLASSES, NUM_TRAIN);

        std::cout << "Loading training data..." << std::endl;
        read_mnist(std::string(FILE_TRAIN_IMAGES), x);
        ...
        /* Split into train set and dev set, you should use train set to train
           your neural network and dev set to evaluate its precision */
        ...
        /* Load the test data, we will compare the prediction of your trained
           neural network with test data label to evaluate its precision */
    ...
    if ((rank == 0) && (run_seq)) {
        ...
        train(seq_nn, x_train, y_train, learning_rate, reg, num_epochs,
    ...
    if (rank == 0) {
        ...
        real prec = precision(label_dev_pred, label_dev);
        std::cout << "Precision on validation set for parallel training = "
                  << prec << std::endl;
